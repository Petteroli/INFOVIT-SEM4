...
public class TetrisView extends JPanel {
    ...
    private void drawGame(Graphics2D g2) {
        ...
        CellPositionToPixelConverter converter = new CellPositionToPixelConverter(box,viewableTetrisModel.getDimension(), CELLMARGIN);

        drawCells(g2, viewableTetrisModel.getTilesOnBoard(), converter, colorTheme);
        drawCells(g2, viewableTetrisModel.getTetrominoCells(), converter, colorTheme);
        if (viewableTetrisModel.getGameState() == GameState.GAME_OVER) {
            g2.setColor(colorTheme.getTransparentLayer());
            g2.fill(box);
        }
